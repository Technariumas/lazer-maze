...
inline void RFM69::ssLow() {
	PORTA &= ~_BV(PA2);
}

inline void RFM69::ssHigh() {
	PORTA |= _BV(PA2);
}
...
void RFM69::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
  ...
  while (getInterruptPinStatus() == 0 && millis() - txStart < RF69_TX_LIMIT_MS); // wait for DIO0 to turn HIGH signalling transmission finish
